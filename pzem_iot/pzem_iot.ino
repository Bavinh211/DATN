...
void read_firebase(void)
{
  ...
  if (count.tran != count.old_tran)
  {
    ...
    /***** hien thi khi gas *****/
    char gas[7];
    sprintf(gas,"%02d", data.gas);
    Firebase.setString(firebaseData, path + "/gas", (String)gas);
...
void read_bnt(void)
{
  ...
  uint16_t adc_value = READ_AO;
  data.gas        = (adc_value/4096.0) *100.0;
  //Serial.println(data.gas);
...
void warning(void)
{
  if (status.warning == 0)
  {
    if (data.voltage > setting.volt || data.current > setting.current || data.gas > setting.gas)
    {
      BUZZER_ON;
      status.warning = 1;
    }
